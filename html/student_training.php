...
<html>
<head>
    <title>Доступные тренировочные тесты
    ...
    <div class="row">
        <div class="col-md-10 well">
            <form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="settingsform">
                <fieldset>
                    <legend>Список тренировочных тестов <span style="font-size: 60%; "><i><a
                                        href="student.php">[перейти к контрольным]</a></i></span></legend>
                    ...
                    if (($result_select = mysqli_query($con, "SELECT * FROM tests WHERE archive!=1 AND mode = 'TRAINING' AND (user_group IS NULL OR user_group = '" . $user_group . "') ORDER BY id DESC"))->num_rows > 0) {
                    ?>
