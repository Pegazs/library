			...
			<div class="col-md-12 well">
           		<form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="settingsform">
					<fieldset>
						...
						<?php
						if (($result_select = mysqli_query($con, "SELECT * FROM `users`,`sessions` WHERE `sessions`.`user_id` = `users`.`id` AND status != 'started' ORDER BY `sessions`.`id` DESC LIMIT 1000")) && ($result_select->num_rows) > 0) {
						?>
	                    ...
	                    ?>
	                    		<tr>
									...
									<td>
										<?php if ($session->user_group == "") { echo "Нет"; } else { echo $session->user_group; }; ?>
									</td>
									<td>
