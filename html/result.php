...
<html>
<head>
	<title>Результаты прохождения теста «<?php echo ($test_info->name) ?>» — <?php echo mysqli_fetch_object(mysqli_query($con, "SELECT settings_value FROM settings WHERE settings_name ='site_name'"))->settings_value ?></title>
           ...
           <form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="usersform">
				<fieldset>
                    ...
                    {      ?>
                    	...
                    	<div class="col-md-6">
                    	<b>Время начала прохождения:</b> <?php echo date('H:i:s (d.m.Y)', strtotime($session_info->start_time)); ?>
                    	</div>
                      	<div class="col-md-6">
                    	<b>Имя пользователя:</b> <?php echo $user_info->name; ?>
                    	</div>
                    	<div class="col-md-6">
                    	<b>Время окончания прохождения:</b> <?php echo date('H:i:s (d.m.Y)', strtotime($session_info->finish_time)); ?>
                    	</div>
                    	<div class="col-md-6">
                    	<b>Группа:</b> <?php if ($user_info->user_group == "") { echo "Отсутствует"; } else { echo $user_info->user_group; } ?>
                    	</div>
                    	<div class="col-md-6">
                    	<b>Результат:</b> <?php echo $session_info->result; ?> <?php if (($session_info->result) >= ($test_info->necessary)) { echo "<font color=#006600>(пройден)</font>"; } else { echo "<font color=#CC0033>(не пройден)</font>"; } ?>
                    	</div>
                    	<br><br>
                       <?php while($questions = mysqli_fetch_object($result_select))
                        ...
                        <tr bgcolor=#ffffff>
                        	<td>
								<b>Вопрос №<?php echo ($questions->order_num) ?>:</b> <?php echo ($questions->text) ?>
							</td>
      					</tr>
      					...
      					<?php }  ?>
                        <tr <?php if (($test_info->disable_show) != 1) { if ($questions->correct) { echo "class='success'"; } else { echo "class='danger'"; } } ?>>
                        	<td>
